...
function alterar(id) {
    ...
    fetch("http://127.0.0.1:5500/caixa/" + id)
        .then(resp => resp.json())
        .then(dados => {
            const txtdata = document.getElementById("txtdata");
            const txtdescricao = document.getElementById("txtdescricao");
...
function listar() {
    ...
    fetch("http://127.0.0.1:5500/caixa?pesquisa=" + txtpesquisa.value)
        .then(resp => resp.json())
...
function excluirSim() {
    fetch("http://127.0.0.1:5500/caixa/" + idatual,
        {
            headers: {
...
function salvar() {
    ...
    var idNovo = 0
    var todosCl = fetch("http://127.0.0.1:5500/caixa?pesquisa=" + txtpesquisa.value)
    ...
    var metodo;
    if (idatual <= 0) {
        url = "http://127.0.0.1:5500/caixa";
        metodo = "POST";
    } else {
        url = "http://127.0.0.1:5500/caixa/" + idatual;
        metodo = "PUT";
    }
    fetch(url,
        {
